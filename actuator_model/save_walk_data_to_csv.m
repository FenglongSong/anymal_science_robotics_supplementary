...
set(gca, 'Position', [50 50 550 230])
    ...
    hold on
    axis([time_start time_end -1.08 -0.13])
...
readData = csvread("data/seaModelValidate_walk.txt");
...
axis([time_start time_end -12 23.5])
...
%% Write LF_KFE joint position to csv
LF_KFE_joint_measure_pos = logElements(idx_pos(idx_)).data;
LF_KFE_joint_command_pos = logElements(idx_com(idx_)).data;
LF_KFE_joint_measure_vel = logElements(idx_vel(idx_)).data;
LF_KFE_joint_measure_tor = logElements(idx_tor(idx_)).data;

csvwrite('data_csv/walk_measure_pos.csv', LF_KFE_joint_measure_pos);
csvwrite('data_csv/walk_command_pos.csv', LF_KFE_joint_command_pos);
csvwrite('data_csv/walk_measure_vel.csv', LF_KFE_joint_measure_vel);
csvwrite('data_csv/walk_measure_tor.csv', LF_KFE_joint_measure_tor);
csvwrite('data_csv/walk_time.csv', time);
